...
// Example usage:
const socket = io('http://your-ngrok-url'); // Replace with your ngrok URL
const webrtc = new Webrtc(socket);

webrtc.addEventListener('createdRoom', (event) => {
    console.log('Created room:', event.detail.roomId);
});

webrtc.addEventListener('joinedRoom', (event) => {
    console.log('Joined room:', event.detail.roomId);
});

webrtc.addEventListener('newUser', (event) => {
    console.log('New user joined:', event.detail.socketId);
...
webrtc.addEventListener('removeUser', (event) => {
    console.log('User left:', event.detail.socketId);
    // Remove the video element for the user who left.
});

// ... rest of your code for handling events and user interactions

// Example to join room:
webrtc.joinRoom('myRoom');

// Example to get local stream and start sharing:
webrtc.getLocalStream().then((stream) => {
    webrtc.gotStream();
    // Add stream to your own video element.
});
